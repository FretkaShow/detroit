surface.CreateFont('ice-lockpick.normal', {
	font = 'Calibri',
	extended = true,
	size = 24,
	weight = 350,
...
netstream.Hook('ice-lockpick', function(ent, key, data)

	local function clear()
		hook.Remove('HUDPaint', 'ice-lockpick')
		hook.Remove('Think', 'ice-lockpick')
		hook.Remove('RenderScreenspaceEffects', 'ice-lockpick')
		hook.Remove('InputMouseApply', 'ice-lockpick')
		hook.Remove('CreateMove', 'ice-lockpick')
		hook.Remove('dbg-view.chPaint', 'ice-lockpick')
		timer.Remove('ice-lockpick')
	end
	...
	if ent then
		...
		local function succ()
			...
			netstream.Start('ice-lockpick', ent, resp)
		end

		local function fail()
			netstream.Start('ice-lockpick', ent, '')
		end
		...
		hook.Add('HUDPaint', 'ice-lockpick', function()
			...
			draw.DrawText(txtHint, 'ice-lockpick.normal', cx + totalW / 2 + 20, cy - 95, color_white, TEXT_ALIGN_LEFT)

		end)
		local lastTime = nil
		hook.Add('Think', 'ice-lockpick', function()
		...
		local suspicion = 0
		local xSum, ySum = 0, 0
		timer.Create('ice-lockpick', 0.5, 0, function()
			...
			if suspicion > 3 then
				clear()
				netstream.Start('ice-lockpick.exploit')
			end
		...
		hook.Add('InputMouseApply', 'ice-lockpick', function(cmd, _x, _y)
			...
			if RealTime() > nextPush and lmbReleased then
				...
				if -_y / FrameTime() > 3000 then
					...
					netstream.Start('ice-lockpick.push')
		...
		hook.Add('CreateMove', 'ice-lockpick', function(cmd)
			...
			if cmd:KeyDown(IN_ATTACK2) then
				netstream.Start('ice-lockpick', nil)
			end
		...
		hook.Add('RenderScreenspaceEffects', 'ice-lockpick', function()
