...
hook.Add('dbg-admin.getActions', 'dbg-admingun', function(menu, ply, id)
	...
	if LocalPlayer():IsAdmin() then
		...
		local sm, pmo = menu:AddSubMenu(L.condition)
			sm:AddOption('Восстановить здоровья', function() RunConsoleCommand('sgs', 'hp', steamID, '100') end)
			...
			sm:AddOption('Убить', function() RunConsoleCommand('sgs', 'slay', steamID) end)
