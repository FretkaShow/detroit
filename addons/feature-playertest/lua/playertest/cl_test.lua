...
Благодаря просторной карте автомобильная инфраструктура развита, а слоты для игроков увеличены]],
	...
	[''] = {
		name = 'Las VenturasRP',
		desc = [[С ходом времени Las Venturas стал расширяться, и у него появился пригород. Вашингтон – современный район города, расположившийся на обширной территории. Здесь есть районный центр, озеро с загородными коттеджами, а также промышленная зона
...
--На этом сервере стоят последние и почти всегда недоделанные обновления, в связи с чем многие вещи еще работают не совсем, как надо. Вход выполняется по вайтлисту, а прогресс не переносится на остальные сервера]],
    ...
    default = {
		name = 'Detroit',
		desc = [[Detroit — город на севере США, в штате Мичиган. Расположен в юго-восточном углу штата, на реке Детройт, на границе с Канадой. Основан 24 июля 1701 года французским приказчиком Антуаном Ломе как фактория для торговли пушниной с индейцами. До XIX века входил в состав Канады (часть Британской империи), потом передан США, на момент передачи представлял собой просто огороженный участок земли.]],
	},
}
...
function test.welcomeScreen(attempts, hint, showMsg)
	...
	local links = octolib.button(bp, 'Ссылки', function()
		local m = DermaMenu()
		m:AddOption(L.rules_server, function() octoesc.OpenURL('https://discord.gg/6KVVGKgBSk') end)
		m:AddOption(L.forum, function() octoesc.OpenURL('нету') end)
		m:AddOption(L.wiki, function() octoesc.OpenURL('https://discord.gg/6KVVGKgBSk') end)
		m:AddOption(L.we_in_steam, function() octoesc.OpenURL('нету') end)
		m:AddOption(L.we_in_vk, function() octoesc.OpenURL('нету') end)
		m:AddOption(L.our_site, function() octoesc.OpenURL('https://discord.gg/6KVVGKgBSk') end)
		m:AddOption(L.write_us_in_vk, function() octoesc.OpenURL('https://discord.gg/6KVVGKgBSk') end)
		m:Open()
	end)
	links:SetWide(115)
	links:Dock(LEFT)
...
local config = {
	...
	rp_unioncity_day = {
		{
			start = { Vector(4480, -318, 5677), Angle(2, 179, 0), 57 },
			finish = { Vector(-722, -330, 5653), Angle(0, 179, 0), 42 },
			time = 20,
		},
		{
			start = { Vector(-4388, 6411, 5658), Angle(0, -0, 0), 60 },
			finish = { Vector(-726, 6385, 5633), Angle(0, -0, 0), 60 },
			time = 20,
		},
		{
			start = { Vector(1617, -3564, 5530), Angle(1, 89, 0), 68 },
			finish = { Vector(1618, -1742, 5493), Angle(1, 89, 0), 55 },
			time = 20,
		},
	},
	default = {{
