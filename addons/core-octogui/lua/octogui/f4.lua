...
function octogui.reloadF4()
	...
	function f:Paint(w, h)
		...
		if GetConVar('octolib_blur'):GetBool() then
			...
			for i = 1, 3 do
				blur:SetFloat('$blur', a * i * 4.5)
				blur:Recompute()

				render.UpdateScreenEffectTexture()
				surface.DrawTexturedRect(-5, -1, w + 2, h + 2)
			end
		else
		...
		drawText(
			L.play_time_hint .. playTime(LocalPlayer():GetTimeTotal()), 'f4.medium', w / 2, 50,
			TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, cols.txt, cols.txtSh
		)
        
		surface.SetAlphaMultiplier(a)
		drawText(
			teamName(ply) .. ' ' .. ply:Name(), 'f4.medium', w / 2, 10,
			TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, cols.txt, cols.txtSh
		)
		...
		drawText(
			...
			L.salary_hint .. DarkRP.formatMoney(ply:Salary()), 'f4.normal', w - 10, h - 8,
			TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, cols.txt, cols.txtSh
		)
		surface.SetAlphaMultiplier(1)
