...
p.Think = function( self )
	...
	if LocalPlayer():IsAdmin() and not IsValid(self.adminBut1) then
		...
		b3.icon = Material(octolib.icons.silk16('camera'))
		b3.Paint = paintTag
		b3.Think = function(self2)
			self2:SetAlpha(self.al * 255)
		end
		b3:AddHint(L.admin_screen)
		function b3:DoClick()
			RunConsoleCommand('gimmethatscreen')
		end
                    
		local b4 = vgui.Create 'DButton'
		b4:SetText('')
		b4:SetSize(24,24)
		b4:SetPos(ScrW() / 2 + 46, ScrH() - 32)
		b4.icon = Material(octolib.icons.silk16('user'))
		b4.Paint = paintTag
		b4.Think = function(self2)
			self2:SetAlpha(self.al * 255)
		end
		b4:AddHint("Панель гейм-мастеров")
		function b4:DoClick()
			RunConsoleCommand('gmpanel')
		end
