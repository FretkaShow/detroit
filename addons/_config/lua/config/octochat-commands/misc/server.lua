...
octochat.registerCommand('/givecert', {
    ...
    execute = function(ply)
        ...
        local curCert = tgt:GetDBVar('cert')
        print(curCert) -- Проверяем текущее удостоверение в консоли

        local issue = luatz.time() -- Дата выдачи удостоверения

        local period = curCert and curCert.period or 0

        -- Вычисляем дату окончания действия удостоверения
        local expiration = issue + period * 86400 -- Период в секундах (1 день = 86400 секунд)

        -- Преобразуем даты в формат для вывода
        local issueDate = luatz.strftime("%d.%m.%Y", luatz.localtime(issue))
        local expirationDate = luatz.strftime("%d.%m.%Y", luatz.localtime(expiration))
        ...
        octolib.request.send(ply, {
            ...
            -- ваша функция для обработки результата
        end)
    end,
})
octochat.registerCommand('/givecert', {
    ...
    execute = function(ply)
        ...
        local issue = luatz.time()
        local curCert = tgt:GetDBVar('cert')

        -- Проверка на наличие текущего удостоверения и его периода
        local period = curCert and curCert.period or 0
        ...
        octolib.request.send(ply, {
            ...
            if dbgCerts.give(tgt, curCert and curCert.id, curCert and curCert.title, data[1], data[2], data[3], issue, rem) then
                ply:Notify('Выдано удостоверение:')
                dbgCerts.show(tgt, ply, true, true)
                dbgCerts.show(tgt, tgt, true, true)
            else
                ply:Notify('warning', 'Не получилось выдать удостоверение')
            end
        end)
    end,
})
